    ...
    for framenum in tqdm.trange(0, maxnumframes):
        ...
        for i, j in enumerate(boxdata):
            if boxdata[i, 0] == framenum and (boxdata[i, 5] - boxdata[i, 3]) * (boxdata[i, 4] - boxdata[i, 2]) > IMG_SIZE_MINIMUM:
                ...
                y1, y2, x1, x2 = boxdata[i, 3], boxdata[i, 5], boxdata[i, 2], boxdata[i, 4]
                if y2 - y1 > x2 - x1:
...
def saveimages(labels, imgdata):
    print('\nSaving Images')
    if not os.path.exists('Files/{}/Clusters{}{}{}/'.format(videoname, threshold, blur, squaresize)):
        os.makedirs('Files/{}/Clusters{}{}{}'.format(videoname, threshold, blur, squaresize))
...
blur = 100             # Pixel grayscale values less than blurring value become black. Lower = higher sensitivity
...
print('Median Values Determined')
...
if os.path.exists('Files/{}/{}{}{}VideoBoxID.txt'.format(videoname, threshold, blur, squaresize)) == False:
    ...
    ConvNet(boxdata)
print('Classification of Images Complete')

print('Complete!!')

# historic box values:
# (1xdif-50 175 60)   Squares too wide
# (1xdif-50 175 1xdif-50)   Doesn't detect enough
# (30 160 1xdif-50)   Detects more. Squares too wide
...
# (8 120 8)     Boxes too big. Need to increase sensitivity and decrase box size
# (7 120 7)     Like Threshold. Try decreasing box size
# (6 120 6)     Connectivity too strong. Too large of boxes
# (5 120 5)     Connectivity too strong. Too large of boxes
# (7 120 5)     Connectivity too strong
# (7 110 5)     Connectivity too strong
# (7 110 3)
# (7 110 2)
# (7 110 1)
# (7 1xdif-100 1)     Square Size still too big. Getting too much info?
# (8 120 0)     Discontinuous
# (8 1xdif-100 0)     Best. S
# (5 150 0)     Discontinuous
