...
input_dimensionality = 4        # num input observations

model_batch_size = 3            # Batch size when learning from model
real_batch_size = 3             # Batch size when learning from real environment

#initiaize variables
observations = tf.placeholder(tf.float32, [None, 2], name="input_x")        # 2 observations
...
W1 = tf.get_variable("W1", shape=[2, num_hidden_layers], initializer=tf.contrib.layers.xavier_initializer())
...
bObsv = tf.Variable(tf.zeros([2]), name="bO")
wObsv = tf.get_variable("wO", shape=[model_layer_size, 2], initializer=tf.contrib.layers.xavier_initializer())
...
true_observation = tf.placeholder(tf.float32, [None, 2], name="true_observation")
