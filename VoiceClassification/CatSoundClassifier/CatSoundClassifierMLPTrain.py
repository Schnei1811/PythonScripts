...
def deep_neural_network_model(x, hiddenlayersize, n_classes, n_features):
    hidden_1_layer = {'f_fum': hiddenlayersize,
                      'weight': tf.Variable(tf.random_normal([n_features, hiddenlayersize])),
                      'bias': tf.Variable(tf.random_normal([hiddenlayersize]))}
    hidden_2_layer = {'f_fum': hiddenlayersize,
                      'weight': tf.Variable(tf.random_normal([hiddenlayersize, hiddenlayersize])),
                      'bias': tf.Variable(tf.random_normal([hiddenlayersize]))}
    output_layer = {'f_fum': None,
                    'weight': tf.Variable(tf.random_normal([hiddenlayersize, n_classes])),
...
with tf.Session() as sess:
    ...
    saver.save(sess, "model/MLPmodel.ckpt")
sess.close()
