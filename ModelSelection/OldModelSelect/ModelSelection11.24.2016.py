...
def rf(rfmodelaccuracy, modelselect, n_classes):
    X, y, train_x, test_x, train_y, test_y = LoadData(modelselect)
    starttime = time.time()
    ...
    ans = input("Would you like to view and save classification distribution scores? (y/n): ")
    if ans in ['y']:
...
def snn(regularization, hm_epochs, hidden_size, degreepoly, snnmodelaccuracy, modelselect):
    ...
    # print("Training...")
    # J, grad = backprop(params, input_size, hidden_size, num_labels, X, train_y_onehot, regularization)

    fmin = minimize(fun=backprop, x0=params, args=(input_size, hidden_size, num_labels, X, train_y_onehot, regularization),
...
def recurrentnn(hm_epochs, degreepoly, rnnmodelaccuracy, n_features, n_classes, modelselect):
    ...
    if featuresPrime == True:
        print("Number of Features Prime. \nAn additional column of 0s has been added to the Data")
        primecounter = 1
        ones = np.zeros((len(train_x), 1))
        train_x = np.append(train_x, ones, axis=1)
        ones = np.zeros((len(test_x), 1))
        test_x = np.append(test_x, ones, axis=1)
    ...
    with tf.Session() as sess:
        ...
        if ans in ["y"]:
            ...
            newmodelaccuracy = open("modelparameters/{}/BiNNmodelaccuracy.txt".format(dataname), "w")
            newmodelaccuracy.write(
...
def recurrent_neural_network(x, n_classes, rnn_size, n_chunks, chunk_size):
...
def use_recurrent_neural_network(input_data):
    ...
    x = tf.placeholder('float', [None, n_chunks, chunk_size])
    rnn_size = 512
    prediction = recurrent_neural_network(x, n_classes, rnn_size, n_chunks, chunk_size)
    saver = tf.train.Saver()
...
def ModelSelection():
    try:
        ...
        try:
            ...
            print('No dataset available with given input name')
            exit()
    ...
    if modelselect in ["knn"]:
        ...
        rf(rfmodelaccuracy, modelselect, n_classes)
...
if ans in ["t"]:
    ...
    dataname = "GestureData"
    predictionname = "GestureTest.txt"

    # df = np.loadtxt('predictiondata/{}'.format(predictionname), delimiter=",")
    # datadescription = np.loadtxt("modelparameters/{}/nclassesfeatures.txt".format(dataname), delimiter=",")
    # n_classes, n_features = int(datadescription[0]), int(datadescription[1])
    ...
    input_data = np.loadtxt('predictiondata/GestureDataTest.txt')
