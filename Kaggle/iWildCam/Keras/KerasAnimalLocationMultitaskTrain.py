...
def VGG16():
    ...
    location_output = Dense(65, activation='softmax', name='location_out')(x)
    animal_output = Dense(2, activation='softmax', name='animal_out')(x)

    # Create model.
    model = Model(inputs=img_input, outputs=[location_output, animal_output], name='vgg16')
...
train_data = np.load('../data/50_pixel_mock_train_multitask_data.npy')
val_data = np.load('../data/50_pixel_mock_val_multitask_data.npy')
...
model.fit_generator(input_generator,
                    ...
                    callbacks=[#EarlyStopping(min_delta=0.001, patience=3),
                     ModelCheckpoint('VGG16_saved_models/1xdif-50/weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss',
                                     verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=5)])


# model.fit(x_train, [y_train_location, y_train_animal], epochs=1, batch_size=32,
#           validation_data=(x_test, [y_test_location, y_test_animal]))

# Save model and weights

# Score trained model.
animal_scores = model.evaluate(x_test, y_test_animal, verbose=1)
location_scores = model.evaluate(x_test, y_test_location, verbose=1)
print('Val loss:', animal_scores[0])
print('Val accuracy:', animal_scores[1])
print('Val loss:', location_scores[0])
print('Val accuracy:', location_scores[1])
